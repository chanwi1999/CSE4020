...
def exp(rv):
    ...
    z = rv[2]
    R = np.identity(3)
    R[0,0] = np.cos(th)+((x*x)*(1-np.cos(th)))
    R[0,1] = ((x*y)*(1-np.cos(th)))-z*np.sin(th)
    R[0,2] = ((x*z)*(1-np.cos(th)))+y*np.sin(th)
    R[1,0] = ((y*x)*(1-np.cos(th)))+z*np.sin(th)
    R[1,1] = np.cos(th)+((y*y)*(1-np.cos(th)))
    R[1,2] = ((y*z)*(1-np.cos(th)))-x*np.sin(th)
    R[2,0] = ((z*x)*(1-np.cos(th)))-y*np.sin(th)
    R[2,1] = ((z*y)*(1-np.cos(th)))+x*np.sin(th)
    R[2,2] = np.cos(th)+((z*z)*(1-np.cos(th)))
    return R

def log(R):
    th = np.arccos((R[0,0]+R[1,1]+R[2,2]-1)/2)
    rv0 = (R[2,1]-R[1,2])/(2*np.sin(th))
    rv1 = (R[0,2]-R[2,0])/(2*np.sin(th))
    rv2 = (R[1,0]-R[0,1])/(2*np.sin(th))
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2018008177-10-1', None,None)
    if not window:
        glfw.terminate()
        return
